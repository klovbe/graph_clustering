...
def com_graph(df, metric):
    if metric == 'pearson':
        ...
        graph = squareform(pdist(df, metric='euclidean'))
    elif metric == 'cosine':
        graph = squareform(pdist(df, metric='cosine'))
    else:
...
def batch_generator_sdne(X, batch_size, shuffle, beta=1):
    ...
    while True:
        batch_index = \
            sample_index[batch_size * counter:min(batch_size * (counter + 1), row_indices.shape[0])]
